...
with tab1:
    ...
    with col3:
        st.metric(label="Nombre de colonnes numériques", value=somme_num_value.sum())
    with col4:
...
with tab3:    
    ...
    with col6:
        st.write("Exemple de valeurs max non conformes dans la wave 6")
        ...
        st.write("Exemple de valeurs max non conformes dans la wave 7")
...
# ages
with tab4:
    st.subheader("Profil des âges masculins et féminins de nos participants")
    age_gender_female = age_gender(0, df)
    fig1, ax1 = plt.subplots()
    ax1.set_ylabel('age')
    ax1.set_title('Outlier des âges féminins')
    ...
    ax2.set_ylabel('age')
    ax2.set_title('Outlier des âges masculins')
...
txt = st.text_area(
    ...
    "On peut voir que le nombre 79 de données manquantes revient souvent dans les différentes colonnes, ces données seront supprimées. "
    "Il est à noter que d'autres données restent manquantes et demanderont une attention particulière. "
    ...
    "En conséquence, il faudra les séparer lors des différents calculs. "
    "Certaines waves de 6 à 9 ne respectent pas la notation de 1 à 10 et seront exclues des calculs car non conformes à la base de notation. "
    ""
    "Beaucoup de données catégorielles ont été converties numériquement et ne pourront être utilisées pour des calculs. "
    "Nous avons une personnes de sexe féminin âgée de 55 ans qui a obtenu plusieurs rendez-vous avec des hommes plus jeunes, peut-être s'agit-il d'une erreur de saisie de son âge. "
